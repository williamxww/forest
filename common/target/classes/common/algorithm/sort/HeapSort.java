 ...
 *
 * 堆是一种重要的数据结构，为一棵完全二叉树, 底层如果用数组存储数据的话，假设某个元素为序号为i(从0开始),
...
public class HeapSort {
    ...
    /**
     * 构造最大堆，此方法可以确保index对应的子树中，所有的根节点一定是最大的
     *
    ...
    private void initMaxHeap(int[] array) {

        // 最后2个节点的序号(array.length-1)，(array.length-2)，他们的父节点是(array.length-2)/2
        // 从最后这个父节点位置往前依此循环,通过maxHeap使得parent上放的值是子树中最大的
        int lastParentIndex = (array.length - 2) / 2;
        for (int parent = lastParentIndex; parent >= 0; parent--) {
            maxHeap(array, array.length, parent);
        }
    }
